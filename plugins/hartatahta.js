let handler = async (m, { Itsuki, text }) => {
    Itsuki.hartatahta = Itsuki.hartatahta ? Itsuki.hartatahta : {}
    if (m.chat in Itsuki.hartatahta) return m.reply('Masih ada yang sedang membuat\nTeks Harta Tahta\ndi chat ini... tunggu sampai selesai')
    else Itsuki.hartatahta[m.chat] = true
    m.reply('_Sedang membuat..._\n*Mohon tunggu sekitar 1 menit*')
    try {
        let img = await ht(text ? text : ':v')
        Itsuki.sendFile(m.chat, img, 'Harta Tahta.png', '*© Nurutomo*\nMade with FFmpeg', m)
    } finally {
        delete Itsuki.hartatahta[m.chat]
    }
}
handler.help = ['tahta']
handler.tags = ['nulis']
...
let src = join(__dirname, '../system/')
let tmp = join(__dirname, '../system/storage/')
