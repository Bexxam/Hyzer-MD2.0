import { smsg } from './system/realize.js'
...
import db, { loadDatabase } from './system/database.js'
import Queque from './system/queque.js'
 ...
 * @this {import('./system/connection').Socket}
    ...
    try {
        ...
        try {
            ...
            if (settings) {
                ...
                if (!('fakejid' in settings)) settings.fakejid = false
        ...
        const isROwner = (config.owner).map(v => v + "@s.whatsapp.net").includes(m.sender)
        const isOwner = isROwner || m.fromMe
        const isMods = isOwner || (config.moderator).map(v => v + "@s.whatsapp.net").includes(m.sender)
        const isPrems = isROwner || (config.premium).map(v => v + "@s.whatsapp.net").includes(m.sender)
        ...
        const q = db.data.settings[this.user.jid] && db.data.settings[this.user.jid].fakejid ? fakeM : m
        ...
        case 'remove':
            if (!chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = './sysytem/media/default.png'
                    try {
                        pp = await this.profilePictureUrl(user, 'image')
                    } catch (e) {
                    } finally {
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || Connection.Itsuki.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '-') :
                            (chat.sBye || this.bye || Connection.Itsuki.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0])
                        let llus = await getBuffer(pp)
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || Connection.Itsuki.spromote || '@user ```is now Admin```')
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || Connection.Itsuki.sdemote || '@user ```is no longer Admin```')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || Connection.Itsuki.sDesc || '```Description has been changed to```\n@desc').replace('@desc', groupUpdate.desc)
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || Connection.Itsuki.sSubject || '```Subject has been changed to```\n@subject').replace('@subject', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || Connection.Itsuki.sIcon || '```Icon has been changed to```').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || Connection.Itsuki.sRevoke || '```Group link has been changed to```\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, Itsuki) => {
    let msg = {
        ...
        query: 'Sertakan query dalam command.'
    }[type]
...
watchFile(file, async () => {
    ...
    if (Connection.reload) console.log(await Connection.reload(await Connection.Itsuki))
})